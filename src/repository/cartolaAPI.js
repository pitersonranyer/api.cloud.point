...
const getParciaisAtletasMercadoAberto = async (time_id) => {
  ...
  if (resultJson.body) {
    ...
    for (let i = 0; i < atletasArray.length; i++) {
      ...
      var ind = 0;
      Object.keys(resultPosicoes.body).forEach((id) => {
        const pos = {
          id: id,
          nome: resultPosicoes.body[id].nome,
          abreviacao: resultPosicoes.body[id].abreviacao,
        };
...
const recuperarDadosAtletas = async (atleta_id, nrRodada, ind, parm_clube_id) => {
  ...
  if (atletas.length > 0) {
    ...
    atletasArray[ind].scoutPositivo = atletas[0].scoutPositivo.replace(/1/gi, '');
    atletasArray[ind].scoutNegativo = atletas[0].scoutNegativo.replace(/1/gi, '');
...
const getPontuacaoAtletasRodada = async (nrRodada) => {
  ...
  atletas = await sequelize.query("SELECT `atletas`.`nrRodada` " +
    ...
    "      FROM `atletas` " +
    "      WHERE `atletas`.`nrRodada` " + `= "${nrRodada}" ` +
    " ORDER BY `atletas`.`pontuacao` DESC "
    , {
      type: sequelize.QueryTypes.SELECT
    });
  ...
  if (atletas.length > 0) {
    ...
    for (let ind = 0; ind < atletas.length; ind++) {
      atletas[ind].scoutPositivo = atletas[ind].scoutPositivo.replace(/1/gi, '');
      atletas[ind].scoutNegativo = atletas[ind].scoutNegativo.replace(/1/gi, '');
      atletas[ind].atleta_ficou_no_banco = false;
    }
  }
  return atletas;
}
